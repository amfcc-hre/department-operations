(function () {
  ...
  function showResult(result, registration) {
    ...
    if (result.status === "checked_in") {
      ...
      box.innerHTML = "<strong>COLLECTION RECORDED</strong><span>" + escapeHtml(name + " · " + meal) + "</span>";
  ...
  async function rpc(action, payload) {
    var response = await client.rpc("ops_kitchen_service", {
      p_session_token:session.session_token,
      p_action:action,
      p_payload:payload || {}
    });
    if (response.error) throw response.error;
    if (response.data && ["unauthorized","locked"].indexOf(response.data.status) >= 0) {
      throw new Error(response.data.message || "Kitchen access has ended.");
    }
  ...
  async function checkIn(inputSource) {
    ...
    if (!registration) {
      showResult({ status:"invalid", message:"Scan a recognised student card or enter a five-digit registration number." },"");
      el("scan-input").select();
      return;
    }
    el("scan-submit").disabled = true;
    el("scan-submit").textContent = "Recording...";
    try {
      var result = await rpc("check_in", {
        service_date:today(),
      ...
      el("scan-submit").textContent = "Record collection";
      focusScanner();
    }
  }
  function loadSession() {
    try { session = JSON.parse(sessionStorage.getItem("amfcc_ops_session") || "null"); }
    catch (error) { session = null; }
    var permitted = session && session.session_token && (
      session.role === "administrator"
      || session.role === "department" && session.department && session.department.slug === "kitchen"
    );
    el("kiosk-access-required").hidden = !!permitted;
    el("kiosk-workspace").hidden = !permitted;
    if (!permitted) return false;
    el("kiosk-session").textContent = session.role === "administrator"
      ? "School Administration scanner session"
      : "Kitchen scanner session";
    return true;
  }
  function bind() {
    document.querySelectorAll("[data-meal]").forEach(function (button) {
      button.addEventListener("click", function () {
        ...
        document.querySelectorAll("[data-meal]").forEach(function (candidate) {
          candidate.classList.toggle("selected",candidate === button);
        });
        refreshTotal().then(focusScanner).catch(function (error) {
    ...
    el("scan-input").addEventListener("keydown",function (event) {
      ...
      if (event.key.length === 1) {
        if (!scanStartedAt || now - scanLastAt > 140) {
          scanStartedAt = now;
          scanKeyCount = 0;
        }
        scanLastAt = now;
    ...
    el("scan-input").addEventListener("blur",function () {
      setTimeout(function () {
        var active=document.activeElement;
        if (!conferenceClosed && !el("kiosk-workspace").hidden && (!active || ["INPUT","SELECT","TEXTAREA"].indexOf(active.tagName)<0)) {
          focusScanner();
        }
      },250);
    });
    el("refresh-total").addEventListener("click",function () {
      refreshTotal().then(focusScanner).catch(function (error) {
        showResult({ status:"error",message:error.message },"");
      });
    });
    document.addEventListener("keydown",function (event) {
      if (conferenceClosed || el("kiosk-workspace").hidden || event.metaKey || event.ctrlKey || event.altKey || event.key.length !== 1 || ["INPUT","TEXTAREA","SELECT"].indexOf(event.target.tagName) >= 0) return;
      var now=Date.now();
      if (!scanStartedAt || now-scanLastAt>140) {
        scanStartedAt=now;
        scanKeyCount=0;
      ...
      scanKeyCount+=1;
      el("scan-input").focus();
      el("scan-input").value+=event.key;
      event.preventDefault();
    });
  }
  function initialise() {
    if (!window.APP_CONFIG || !window.supabase) {
      el("kiosk-access-required").hidden=false;
      el("kiosk-access-required").querySelector("p").textContent="The Operations connection is not configured.";
      return;
    }
